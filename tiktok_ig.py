...
'''
    Git: https://git-scm.com/downloads (Debian : # apt-get install git)
...
ig_accounts = ["accountfortesting002", "accountfortesting03", "accountfortesting04", "accountfortesting07", "accountfortesting08", "accountfortesting10"]
ig_passwords = ["Andrea12345678", "Andrea12345678", "Andrea1234", "Andrea12345678", "Andrea12345678", "Andrea12345678"]
ig_index = 0 # index of current ig_account logged in
logged = False
ig_api = None

def IG_login(login_attempt, max_retry = 5):
    global logged, ig_index, ig_accounts, ig_passwords, ig_api
    if login_attempt < max_retry:
        ig_api = Client(ig_accounts[ig_index], ig_passwords[ig_index])
        ig_api.login()
        logged = True
    else:
        sys.exit("LOGIN ERROR! You have reached the max number of attempts")
...
def getIGFollowerCount(username, max_req_per_account=120, default_wait=6, variation=2, max_retry=3):
    ...
    while retry < max_retry:
        try:
            if not logged:
                login_attempt += 1
                IG_login(login_attempt)
            login_attempt = -1
            requests_counter += 1
            retry += 1
            ig_user = {"username_ig": username}
            user = ig_api.username_info(username)
            ig_user["status"] = user["status"]
            ig_user["social_id"] = user["user"]["pk"]
            ig_user["follower_count"] = user["user"]["follower_count"]
            ig_user["following_count"] = user["user"]["following_count"]
            ig_user["media_count"] = user["user"]["media_count"]
            ig_user["usertags_count"] = user["user"]["usertags_count"]
            ig_user["total_igtv_videos"] = user["user"]["total_igtv_videos"]
            break
        except errors.ClientError as e:
            logger.error(e.msg + " (USERNAME: " + str(username) + ", ACCOUNT: " + str(ig_accounts[ig_index]) + (", LOGIN_ERROR)" if not logged else ")"))
            if e.msg == "checkpoint_required":
                ig_index = (ig_index + 1) % len(ig_accounts)
            elif e.msg == "Not Found: user_not_found" or e.msg == "Not Found":
                ig_user["status"] = "not found"
                break
            elif e.msg == "challenge_required":
...
setup_logger('execution_log','./execution.log')
...
def index_doc(index_name, tt_user, id_doc=None, upload_username_IG=False):
    ...
    while not done and retry > 0:
        try:
            ...
            return response
        except (ConnectionTimeout, ConnectionError, AuthenticationException, AuthorizationException, SerializationError,
                TransportError) as ce:
...
def default_variable(data={}):
    ...
    items["device_id"] = str(random.randint(10000, 999999999))
...
dict_users = load_pickle("./", "id_users")
...
url_musically = 'https://api2.musical.ly/aweme/v1/discover/search/'
list_infos = list()
        ...
        while not go_on:
            ...
            while not found and not end_pagination:
                ...
                if j.get('status_msg') == 'Accedi prima col tuo account' or j.get('status_msg') == 'Oops, il tuo account è stato temporaneamente sospeso.' \
                    ...
                    params_musically['device_id'] = str(random.randint(10000, 999999999))
            ...
            #                sys.exit(-1)
                ...
                save_pickle(list_infos, './', 'list_user_infos')
                count += 1
                if count % 100 == 0:
...
def load_username_IG_on_ES():
    ...
    # "./", "list_user_infos.pkl")
    tt_users = load_pickle("./", "list_user_infos")
